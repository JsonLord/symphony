...
- **Kanboard Sync Engine**: Automatically replicates created projects and Plandex-generated tasks to an external Kanboard instance hosted on Hugging Face Spaces via the configured `KANBOARD_API_URL`. State changes (like task completions triggered by Webhooks) are also continuously synchronized to the Kanboard API using `PUT` requests, ensuring external dashboard fidelity. Kanboard tracker links are dynamically forwarded to the Jules agent for context.
